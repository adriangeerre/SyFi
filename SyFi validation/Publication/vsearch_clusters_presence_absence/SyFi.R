...
library(Rsamtools)
bam_data <- readGAlignments("/home/gijs_selten/Fingerprint_2022_12/2025/GCF_004124315/GCF_004124315_ordered.bam")
df <- as.data.frame(bam_data)

ggplot(df, aes(x = start, xend = end, y = seqnames)) +
  geom_segment(aes(yend = seqnames), color = "blue", size = 1) +
  theme_minimal() +
  labs(title = "Read Alignments", x = "Genomic Position", y = "Chromosome")
...
groups <- c("GCF_004124315","GCF_016772275","GCF_018582665","GCF_030123445","GCF_030167035","GCF_032818155","GCF_016724865","GCF_017809215","GCF_030123165","GCF_030123485","GCF_030292135")


for (metric in groups) {
  bam_file <- paste("/home/gijs_selten/Fingerprint_2022_12/2025/GCFs/",metric,"_ordered.bam", sep ="")
  #bam_data <- readGAlignments("/home/gijs_selten/Fingerprint_2022_12/2025/GCF_004124315/GCF_004124315_ordered.bam")
  #df <- as.data.frame(bam_data)
  reference_fasta <- paste("/home/gijs_selten/Fingerprint_2022_12/2025/GCFs/", metric,".fasta", sep ="")
  indexBam(paste("/home/gijs_selten/Fingerprint_2022_12/2025/GCFs/", metric,"_ordered.bam", sep =""))
  chromosome <- "fasta_1"  
  custom_genome <- paste(metric)
  start_pos <- 1
  end_pos <- 1580

  vcf_file <- paste("/home/gijs_selten/Fingerprint_2022_12/2025/GCFs/", metric,".vcf", sep = "")
  vcf <- readVcf(vcf_file, "custom_genome")
  snp_positions <- rowRanges(vcf)  # Extract SNP positions
  snpTrack <- AnnotationTrack(snp_positions, genome="custom_genome", name="SNPs", 
                            col="red", fill="red", stacking="dense", shape = "box", cex =2)

  alignmentTrack <- AlignmentsTrack(bam_file, genome=custom_genome, chromosome=chromosome, name="Mapped Reads")
  plotTracks(list(alignmentTrack, snpTrack), from=start_pos, to=end_pos)

  assign(paste("alignment_plot",metric,sep =""),plotTracks(list(alignmentTrack, snpTrack), from=start_pos, to=end_pos))
}


###SyFi Human microbiome =====
table <- read.table("/media/wolf1/InRoot/Gijs_Selten/fingerprint_manuscript/022023/SyFi_human_microbes/SyFi_human_16S.txt", header=T, sep="\t")
taxonomy = read.table("/home/gijs_selten/Fingerprint_2022_12/2025/SyFi_human_taxonomy.tsv", header=T,sep="\t",quote="\"", fill = FALSE)
derep <- read.table("/media/wolf1/InRoot/Gijs_Selten/fingerprint_manuscript/022023/SyFi_human_microbes/derep_list_SyFi_human.txt")
...
pdf(paste(results.dir,"Figure_3_human_2.pdf", sep=""), width=13, height=13)
print(figure_combined)
dev.off()
