...
###################### After Review ##########################
###Figure S2 - 16S copy number and haplotypes using direct BLAST using 500 filter and adding human dataset =====

#Human
table_16S_human <- read.table(paste(working_directory, "16S_BLAST_2/16S_human_blast.txt", sep = ""), header =F, sep = "\t")
...
#These scripts generate the 16S copy number and haplotype distribution of the 447 AtSC-isolates when blasting directly from the genome assembly instead of using SyFi module 1.
table_16S_plant <- read.table(paste(working_directory, "16S_BLAST_2/16S_plant_blast.txt", sep = ""), header =F, sep = "\t")
...
pdf(paste(results.dir,"Figure_S2_test.pdf", sep=""), width=18, height=6)
print(figure)
dev.off()



###Figure 4 genus level - NRMSE =====
#Function relative abundance (calculates relative abundance per sample from a long format table)
Hank_the_normalizer <- function(df,group,amount){
  df_2 <- df %>% dplyr::group_by_at(group) %>% dplyr::summarise(total=sum(.data[[amount]]))
  df_3 <- df_2$total
...
for (metric in vector) {
  ...
  #Insert dataset for comparison (SyFi, or Qiime2-VSEARCH-generated table)
  otu_table = read.table(paste0(working_directory, "other_tools/AtSC_", metric, ".tsv",sep =""), header=T, sep="\t")
...
data_tog_3$Method_2 <- gsub("100","100%",data_tog_3$Method_2)
data_tog_3$Method_2 <- gsub("99","99%",data_tog_3$Method_2)
data_tog_3$Method_2 <- gsub("98","98%",data_tog_3$Method_2)
data_tog_3$Method_2 <- gsub("97","97%",data_tog_3$Method_2)
data_tog_3$Method_2 <- gsub("95","95%",data_tog_3$Method_2)
...
data_tog_3$Method_2 <- factor(data_tog_3$Method_2, levels = c("95%","97%","98%","99%","100%","Kraken2:Amplicon","Kraken3:Amplicon",
                                                          "Kraken5:Amplicon","Kraken2:Fingerprint","Kraken3:Fingerprint",
                                                          "Kraken5:Fingerprint","Mothur","Salmon","SyFi"))
...
pdf(paste(results.dir,"Figure_4_NRMSE.pdf", sep=""), width=15, height=25)
print(together_plot)
dev.off()
...
No_clusters = read.table(paste(working_directory,"No_clusters_2.txt", sep =""), header=T,sep="\t",quote="\"", fill = FALSE)
No_clusters$vector <- gsub("V3-V4","V3V4", No_clusters$vector)
No_clusters$vector <- gsub("V5-V7","V5V7", No_clusters$vector)
...
correlations$vector <- gsub("_V5V7", "%", correlations$vector)
correlations$vector <- gsub("Salmon%", "Salmon", correlations$vector)
correlations$vector <- gsub("Amplicon%", "Amplicon", correlations$vector)
correlations$vector <- gsub("Fingerprint%", "Fingerprint", correlations$vector)
...
correlations$Group[correlations$vector %in% c("100%","99%","98%","97%","95%")] <- "Qiime2-VSEARCH"
correlations$Group[grep("Amplicon", correlations$Group)] <- "Kraken2 - Amplicon"
correlations$Group[grep("Fingerprint", correlations$Group)] <- "Kraken2 - Fingerprint"

correlations$vector <- factor(correlations$vector, levels = c("95%","97%","98%","99%","100%","Kraken2:Amplicon","Kraken3:Amplicon",
                                                              "Kraken5:Amplicon","Kraken2:Fingerprint","Kraken3:Fingerprint",
                                                              "Kraken5:Fingerprint","Mothur","Salmon","SyFi"))
...
pdf(paste(results.dir,"Figure_5_trial.pdf", sep=""), width=14, height=5)
print(plot_occ)
dev.off()
...
for (metric in vector) {
  #Insert metagenome table
  otu_meta = read.table(paste(working_directory, "other_tools_pres_abs/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t")
  
  row.names(otu_meta) <- otu_meta$isolate
  otu_meta <- otu_meta %>% dplyr::select(-isolate)
  
  #Insert table for comparison
  otu_table = read.table(paste0(working_directory,"other_tools_pres_abs/AtSC_", metric, ".tsv",sep =""), header=T, sep="\t")
...
plot_occ <- ggscatter(Time_table, x = "Duration", y = "File.size..MB.", color = "Threads") + 
  ...
  #ggline(Time_table, x = "Duration", y = "File.size..MB.", color = "Threads") +
  scale_color_manual(values =c("#d55e00","#cc79a7","#0072b2","#f0e442","#009e73","#8BB5E7") ) +
...
pdf(paste(results.dir,"Figure_Time_trial.pdf", sep=""), width=10, height=6)
print(plot_occ)
dev.off()
...
reads_V3V4$Sample <- factor(reads_V3V4$Sample, levels = c("95%", "97%", "98%","99%", "100%", "Kraken2:Amplicon", "Kraken3:Amplicon","Kraken5:Amplicon","Kraken2:Fingerprint","Kraken3:Fingerprint","Kraken5:Fingerprint","Mothur","Salmon","SyFi"))
reads_V5V7$Sample <- factor(reads_V5V7$Sample, levels = c("95%", "97%", "98%","99%", "100%", "Kraken2:Amplicon", "Kraken3:Amplicon","Kraken5:Amplicon","Kraken2:Fingerprint","Kraken3:Fingerprint","Kraken5:Fingerprint","Mothur","Salmon","SyFi"))
...
for(compartment in unique(reads_V3V4$Compartment)) {
  ...
  # Perform ANOVA
  fitAnova <- aov(Percentage ~ Sample, data=reads_V3V4_sub)
  
  # Perform Tukey's post-hoc test
  Tukey <- TukeyHSD(fitAnova)
  
  # Get letters
  letters_anova <- multcompView::multcompLetters4(fitAnova, Tukey)$Sample$Letters
...
ltlbl_combined <- do.call(rbind, lapply(names(anova_results), function(compartment) {
  data.frame(Compartment = compartment, Sample = names(anova_results[[compartment]]), Letters = anova_results[[compartment]])
}))
# Order factors based on your original setup
ltlbl_combined$Compartment <- factor(ltlbl_combined$Compartment, levels = c("Input", "Endosphere", "Rhizosphere"))
ltlbl_combined$Sample <- factor(ltlbl_combined$Sample, levels =  c("95%", "97%", "98%","99%", "100%", "Kraken2:Amplicon", "Kraken3:Amplicon","Kraken5:Amplicon","Kraken2:Fingerprint","Kraken3:Fingerprint","Kraken5:Fingerprint","Mothur","Salmon","SyFi"))
...
ltlbl_combined_2 <- do.call(rbind, lapply(names(anova_results_2), function(compartment) {
  data.frame(Compartment = compartment, Sample = names(anova_results_2[[compartment]]), Letters = anova_results_2[[compartment]])
}))
# Order factors based on your original setup
ltlbl_combined_2$Compartment <- factor(ltlbl_combined_2$Compartment, levels = c("Input", "Endosphere", "Rhizosphere"))
ltlbl_combined_2$Sample <- factor(ltlbl_combined_2$Sample, levels =  c("95%", "97%", "98%","99%", "100%", "Kraken2:Amplicon", "Kraken3:Amplicon","Kraken5:Amplicon","Kraken2:Fingerprint","Kraken3:Fingerprint","Kraken5:Fingerprint","Mothur","Salmon","SyFi"))
...
##############################################################################
###Split kraken2 tables =====
otu_table_kraken2 = read.table(paste0(working_directory,"other_tools_pres_abs/AtSC_kraken2.tsv",sep =""), header=T, sep="\t", row.names =1)
otu_table_kraken2_V3V4 <- otu_table_kraken2[,grep("v3_v4",colnames(otu_table_kraken2))]
otu_table_kraken2_V5V7 <- otu_table_kraken2[,grep("v5_v7",colnames(otu_table_kraken2))]
write.table(otu_table_kraken2_V3V4, paste(working_directory,"other_tools_pres_abs/AtSC_kraken2_V3V4.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")
write.table(otu_table_kraken2_V5V7, paste(working_directory,"other_tools_pres_abs/AtSC_kraken2_V5V7.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")

otu_table_kraken3 = read.table(paste0(working_directory,"other_tools_pres_abs/AtSC_kraken3.tsv",sep =""), header=T, sep="\t", row.names =1)
otu_table_kraken3_V3V4 <- otu_table_kraken3[,grep("v3_v4",colnames(otu_table_kraken3))]
otu_table_kraken3_V5V7 <- otu_table_kraken3[,grep("v5_v7",colnames(otu_table_kraken3))]
write.table(otu_table_kraken3_V3V4, paste(working_directory,"other_tools_pres_abs/AtSC_kraken3_V3V4.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")
write.table(otu_table_kraken3_V5V7, paste(working_directory,"other_tools_pres_abs/AtSC_kraken3_V5V7.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")

otu_table_kraken5 = read.table(paste0(working_directory,"other_tools_pres_abs/AtSC_kraken5.tsv",sep =""), header=T, sep="\t", row.names =1)
otu_table_kraken5_V3V4 <- otu_table_kraken5[,grep("v3_v4",colnames(otu_table_kraken5))]
otu_table_kraken5_V5V7 <- otu_table_kraken5[,grep("v5_v7",colnames(otu_table_kraken5))]
write.table(otu_table_kraken5_V3V4, paste(working_directory,"other_tools_pres_abs/AtSC_kraken5_V3V4.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")
write.table(otu_table_kraken5_V5V7, paste(working_directory,"other_tools_pres_abs/AtSC_kraken5_V5V7.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")


###Adding the SynCom members that are missing and providing 0's - kraken2 - pres/abs - 447 comparison =====
otu_table_kraken2_V3V4 <- read.table(paste(working_directory,"other_tools_pres_abs/AtSC_kraken2_V3V4.tsv", sep =""), header = T, row.names =1, sep = "\t")
otu_meta <- read.table(paste(working_directory, "comparison_datasets/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t", row.names =1)

missing_names <- row.names(otu_meta)[!row.names(otu_meta) %in% row.names(otu_table_kraken2_V3V4)]
zero_df <- data.frame(matrix(0, nrow = length(missing_names), ncol = length(colnames(otu_table_kraken2_V3V4))))
row.names(zero_df) <- missing_names
colnames(zero_df) <- colnames(otu_table_kraken2_V3V4)

otu_table_kraken2_V3V4 <- rbind(otu_table_kraken2_V3V4, zero_df)

write.table(otu_table_kraken2_V3V4, paste(working_directory,"other_tools_pres_abs/AtSC_kraken2_V3V4_2.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")

otu_table_kraken2_V5V7 <- read.table(paste(working_directory,"other_tools_pres_abs/AtSC_kraken2_V5V7.tsv", sep =""), header = T, row.names =1, sep = "\t")
otu_meta <- read.table(paste(working_directory, "comparison_datasets/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t", row.names =1)

missing_names <- row.names(otu_meta)[!row.names(otu_meta) %in% row.names(otu_table_kraken2_V5V7)]
zero_df <- data.frame(matrix(0, nrow = length(missing_names), ncol = length(colnames(otu_table_kraken2_V5V7))))
row.names(zero_df) <- missing_names
colnames(zero_df) <- colnames(otu_table_kraken2_V5V7)

otu_table_kraken2_V5V7 <- rbind(otu_table_kraken2_V5V7, zero_df)

write.table(otu_table_kraken2_V5V7, paste(working_directory,"other_tools_pres_abs/AtSC_kraken2_V5V7_2.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")


otu_table_kraken3_V3V4 <- read.table(paste(working_directory,"other_tools_pres_abs/AtSC_kraken3_V3V4.tsv", sep =""), header = T, row.names =1, sep = "\t")
otu_meta <- read.table(paste(working_directory, "comparison_datasets/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t", row.names =1)

missing_names <- row.names(otu_meta)[!row.names(otu_meta) %in% row.names(otu_table_kraken3_V3V4)]
zero_df <- data.frame(matrix(0, nrow = length(missing_names), ncol = length(colnames(otu_table_kraken3_V3V4))))
row.names(zero_df) <- missing_names
colnames(zero_df) <- colnames(otu_table_kraken3_V3V4)

otu_table_kraken3_V3V4 <- rbind(otu_table_kraken3_V3V4, zero_df)

write.table(otu_table_kraken3_V3V4, paste(working_directory,"other_tools_pres_abs/AtSC_kraken3_V3V4_2.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")

otu_table_kraken3_V5V7 <- read.table(paste(working_directory,"other_tools_pres_abs/AtSC_kraken3_V5V7.tsv", sep =""), header = T, row.names =1, sep = "\t")
otu_meta <- read.table(paste(working_directory, "comparison_datasets/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t", row.names =1)

missing_names <- row.names(otu_meta)[!row.names(otu_meta) %in% row.names(otu_table_kraken3_V5V7)]
zero_df <- data.frame(matrix(0, nrow = length(missing_names), ncol = length(colnames(otu_table_kraken3_V5V7))))
row.names(zero_df) <- missing_names
colnames(zero_df) <- colnames(otu_table_kraken3_V5V7)

otu_table_kraken3_V5V7 <- rbind(otu_table_kraken3_V5V7, zero_df)

write.table(otu_table_kraken3_V5V7, paste(working_directory,"other_tools_pres_abs/AtSC_kraken3_V5V7_2.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")

otu_table_kraken5_V3V4 <- read.table(paste(working_directory,"other_tools_pres_abs/AtSC_kraken5_V3V4.tsv", sep =""), header = T, row.names =1, sep = "\t")
otu_meta <- read.table(paste(working_directory, "comparison_datasets/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t", row.names =1)

missing_names <- row.names(otu_meta)[!row.names(otu_meta) %in% row.names(otu_table_kraken5_V3V4)]
zero_df <- data.frame(matrix(0, nrow = length(missing_names), ncol = length(colnames(otu_table_kraken5_V3V4))))
row.names(zero_df) <- missing_names
colnames(zero_df) <- colnames(otu_table_kraken5_V3V4)

otu_table_kraken5_V3V4 <- rbind(otu_table_kraken5_V3V4, zero_df)

write.table(otu_table_kraken5_V3V4, paste(working_directory,"other_tools_pres_abs/AtSC_kraken5_V3V4_2.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")

otu_table_kraken5_V5V7 <- read.table(paste(working_directory,"other_tools_pres_abs/AtSC_kraken5_V5V7.tsv", sep =""), header = T, row.names =1, sep = "\t")
otu_meta <- read.table(paste(working_directory, "comparison_datasets/AtSC_meta_norm.tsv", sep =""), header=T, sep="\t", row.names =1)

missing_names <- row.names(otu_meta)[!row.names(otu_meta) %in% row.names(otu_table_kraken5_V5V7)]
zero_df <- data.frame(matrix(0, nrow = length(missing_names), ncol = length(colnames(otu_table_kraken5_V5V7))))
row.names(zero_df) <- missing_names
colnames(zero_df) <- colnames(otu_table_kraken5_V5V7)

otu_table_kraken5_V5V7 <- rbind(otu_table_kraken5_V5V7, zero_df)

write.table(otu_table_kraken5_V5V7, paste(working_directory,"other_tools_pres_abs/AtSC_kraken5_V5V7_2.tsv", sep =""), col.names = T, row.names =T, quote =F, sep = "\t")



###Figure 4 species level - NRMSE ======
#Function relative abundance (calculates relative abundance per sample from a long format table)
Hank_the_normalizer <- function(df,group,amount){
  df_2 <- df %>% dplyr::group_by_at(group) %>% dplyr::summarise(total=sum(.data[[amount]]))
  df_3 <- df_2$total
...
vector <- c("95_V3V4", "97_V3V4", "98_V3V4", "99_V3V4", "100_V3V4", 
            "Salmon_V3V4", "SyFi_V3V4","Mothur_V3V4",
            "Kraken2-Genome_V3V4","Kraken3-Genome_V3V4","Kraken5-Genome_V3V4",
            "Kraken2-Amplicon_V3V4","Kraken3-Amplicon_V3V4","Kraken5-Amplicon_V3V4",
            "Kraken2-Fingerprint_V3V4","Kraken3-Fingerprint_V3V4","Kraken5-Fingerprint_V3V4",
            "95_V5V7", "97_V5V7", "98_V5V7", "99_V5V7", "100_V5V7", 
            "Salmon_V5V7","SyFi_V5V7", "Mothur_V5V7",
            "Kraken2-Genome_V5V7","Kraken3-Genome_V5V7","Kraken5-Genome_V5V7",
            "Kraken2-Amplicon_V5V7","Kraken3-Amplicon_V5V7","Kraken5-Amplicon_V5V7",
            "Kraken2-Fingerprint_V5V7","Kraken3-Fingerprint_V5V7","Kraken5-Fingerprint_V5V7")
...
plot_violin <- ggplot(data_tog_3, aes(x = Method, y = NRMSE, colour = Method)) + 
  geom_violin(scale = "width") +
  geom_jitter(
    aes(size = Relative_abundance), 
    shape = 16, 
...
###Figure 4 Genus level - Taxon sensitivity ======
taxonomy = read.table(paste(working_directory,"AtSC_taxonomy_GTDB.tsv", sep =""), header=T,sep="\t",quote="\"", fill = FALSE)
...
###Figure 4 genus level - Taxon specificity =====
#Function relative abundance (calculates relative abundance per sample from a long format table)
Hank_the_normalizer <- function(df,group,amount){
  df_2 <- df %>% dplyr::group_by_at(group) %>% dplyr::summarise(total=sum(.data[[amount]]))
  df_3 <- df_2$total
...
for (metric in vector) {
  ...
  for (genus in unique(fused_table_3$Genus)){
    ...
    f <- function(x, p, alpha = 0.1) {
      exp(-((x - p)^2) / (alpha * p^2))
    }
    
    #fused_table_3_sub$Taxon_specificity <- 2.71828^(-1*200*(fused_table_3_sub$metric_abundances-fused_table_3_sub$metagenome_abundances)^2)
    fused_table_3_sub$Taxon_specificity <- f(fused_table_3_sub$metric_abundances,fused_table_3_sub$metagenome_abundances)
    value_sub_av <- sum(fused_table_3_sub$metagenome_abundances)/length(fused_table_3_sub$metagenome_abundances)
    
    value_sub <- sum(fused_table_3_sub$Taxon_specificity)/length(fused_table_3_sub$Taxon_specificity)
...
row.names(data_tog_3) <- NULL
colnames(data_tog_3) <- c("Genus", "Taxon_specificity","Relative_abundance","Metric")

data_tog_3$Amplicon <- sapply(strsplit(data_tog_3$Metric, "_"),"[[",2)
data_tog_3$Method <- sapply(strsplit(data_tog_3$Metric, "_"),"[[",1)

data_tog_3$Taxon_specificity <- as.numeric(data_tog_3$Taxon_specificity)
data_tog_3$Relative_abundance <- as.numeric(data_tog_3$Relative_abundance)

plot_violin <- ggplot(data_tog_3, aes(x = Method, y = Taxon_specificity, colour = Method)) + 
  geom_violin(scale = "width") +
  geom_jitter(
    aes(size = Relative_abundance), 
    shape = 16, 
  ...
  ggtitle("Genus - Taxon specificity") +
  guides(colour = guide_legend(title = "Method")) +
  facet_wrap(~Amplicon, scales = "free_x")

plot_violin

pdf(paste(results.dir,"Figure_4_Tax_spec_my_idea.pdf", sep=""), width=16, height=10)
print(plot_violin)
dev.off()

###Figure 4 species level - Taxon specificity ======
#Function relative abundance (calculates relative abundance per sample from a long format table)
Hank_the_normalizer <- function(df,group,amount){
  df_2 <- df %>% dplyr::group_by_at(group) %>% dplyr::summarise(total=sum(.data[[amount]]))
  df_3 <- df_2$total
...
#Vector of each dataset - V3V4 or V5V7 amplicons/fingerprints - Qiime2-VSEARCH clustering at 95%, 97%, 98%, 99%, 100% and SyFi (Salmon pseudoalignment of amplicon reads to SyFi-generated fingerprints)
vector <- c("95_V3V4", "97_V3V4", "98_V3V4", "99_V3V4", "100_V3V4", "Salmon_V3V4", "SyFi_V3V4","Mothur_V3V4","Kraken2_V3V4","Kraken3_V3V4","Kraken5_V3V4","95_V5V7", "97_V5V7", "98_V5V7", "99_V5V7", "100_V5V7", "Salmon_V5V7","SyFi_V5V7", "Mothur_V5V7","Kraken2_V5V7","Kraken3_V5V7","Kraken5_V5V7")
...
for (metric in vector) {
  ...
  for (species in unique(fused_table_3$Species)){
    ...
    data_tog <- data.frame(t(data.frame(c(paste(species), value_sub,value_sub_av))))
    data_tog_2 <- rbind(data_tog_2, data_tog)
  }
...
row.names(data_tog_3) <- NULL
colnames(data_tog_3) <- c("Species", "Taxon_specificity","Relative_abundance","Metric")
...
plot_violin <- ggplot(data_tog_3, aes(x = Method, y = Taxon_specificity, colour = Method)) + 
  ...
  ggtitle("Species - Taxon specificity") +
  guides(colour = guide_legend(title = "Method")) +
  facet_wrap(~Amplicon, scales = "free_x")
...
###Figure 4 strain level - Taxon specificity =====
...
for (metric in vector) {
  ...
  for (strain in unique(fused_table_3$Strain)){
    ...
    data_tog <- data.frame(t(data.frame(c(paste(strain), value_sub,value_sub_av))))
    data_tog_2 <- rbind(data_tog_2, data_tog)
  }
...
row.names(data_tog_3) <- NULL
colnames(data_tog_3) <- c("Cluster", "Taxon_specificity","Relative_abundance","Metric")
...
plot_violin <- ggplot(data_tog_3, aes(x = Method, y = Taxon_specificity, colour = Method)) + 
  ...
  ggtitle("Strain - Taxon specificity") +
  guides(colour = guide_legend(title = "Method")) +
  facet_wrap(~Amplicon, scales = "free_x")
...
###Figure 4 Genus level - original taxon specificity =====
#Function relative abundance (calculates relative abundance per sample from a long format table)
Hank_the_normalizer <- function(df,group,amount){
  df_2 <- df %>% dplyr::group_by_at(group) %>% dplyr::summarise(total=sum(.data[[amount]]))
  df_3 <- df_2$total
...
for (metric in vector) {
  ...
  for (genus in unique(fused_table_3$Genus)){
    ...
    fused_table_3_sub$Taxon_specificity <- fused_table_3_sub$metric_abundances/fused_table_3_sub$metagenome_abundances
    fused_table_3_sub$Taxon_specificity[fused_table_3_sub$Taxon_specificity > 1] <- 1
...
pdf(paste(results.dir,"Figure_4_Tax_spec.pdf", sep=""), width=16, height=10)
print(plot_violin)
dev.off()
